...
import { useUserForm, type UserFormData } from '../hooks/useUserForm';
...
function UserDetail() {
  ...
  const initialFormData = useMemo<Partial<UserFormData>>(() => {
    if (isCreateMode) {
      return {
        name: '',
        email: '',
  ...
  const handleSuccess = (_updatedUser: User) => {
    if (isCreateMode) {
      setTimeout(() => {
        navigate('/users');
      }, 1500);
  ...
  const {
    ...
    handleSubmit,
    resetForm,
  } = useUserForm(initialFormData, isCreateMode, id, handleSuccess);
